        ...
        for idx, image in enumerate(images):
            ...
            try:
                if isinstance(image, (str, BinaryIO)):
                    image_path = image if isinstance(image, str) else "BinaryIO"
                ...
                # Log corrupted image and re-raise to skip the entire sample
                logger.warning(f"Found corrupted image (index: {idx}, path: {image_path}): {str(e)}")
                raise OSError(f"Corrupted image at index {idx} (path: {image_path})") from e
