...
class DatasetConverter:
    ...
    def _find_medias(self, medias: Union["MediaType", list["MediaType"], None]) -> Optional[list["MediaType"]]:
        ...
        if self.dataset_attr.load_from in ["script", "file"]:
            # 优先使用数据集特定的 media_dir，否则使用全局的 media_dir
            media_dir = self.dataset_attr.dataset_media_dir or self.data_args.media_dir
                ...
                for i in range(len(medias)):
                    ...
                    if os.path.isfile(media_path):
                        ...
                        logger.warning_rank0_once(
                            f"Media {medias[i]} does not exist in `media_dir` ({media_dir}). Use original path."
                        )
                ...
                for i in range(len(medias)):
                    for j in range(len(medias[i])):
                        ...
                        if os.path.isfile(media_path):
                            ...
                            logger.warning_rank0_once(
                                f"Media {medias[i][j]} does not exist in `media_dir` ({media_dir}). Use original path."
                            )
