...
def _process_multi_modal_data(
    ...
    # video 在 generate_sequences() 里单独处理，这里保持 image 路径不变
    return None
...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        batch_mm_processor_kwargs = non_tensor_batch.pop("mm_processor_kwargs", None)
        if batch_size != len(batch_raw_prompt_ids):
            raise RuntimeError("vllm sharding manager is not work properly.")

        if batch_multi_modal_data is not None:
            vllm_inputs = []
            for raw_prompt_ids, multi_modal_data, mm_kwargs in zip(
                batch_raw_prompt_ids, batch_multi_modal_data, batch_mm_processor_kwargs or [None] * batch_size
            ):
                # image 路径保持原样；video 路径在这里单独映射 key 并透传 mm_processor_kwargs
                if multi_modal_data is not None and "videos" in multi_modal_data and len(multi_modal_data["videos"]) != 0:
                    # video
                    vllm_input = {
                        "prompt_token_ids": list(raw_prompt_ids),
                        "multi_modal_data": {"video": multi_modal_data["videos"]},
                    }
        ...
        if batch_multi_modal_data is not None:
            ...
            if batch_mm_processor_kwargs is not None:
                non_tensor_batch["mm_processor_kwargs"] = batch_mm_processor_kwargs
        else:
            non_tensor_batch = {}
