...
from ..utils.fsdp_utils import (get_fsdp_wrap_policy, get_init_fn,
                                load_fsdp_model, load_fsdp_optimizer,
                                offload_fsdp_model, offload_fsdp_optimizer)
...
from .config import (ActorConfig, CriticConfig, FSDPConfig, ModelConfig,
                     OptimConfig, WorkerConfig)
        ...
        if "multi_modal_inputs" not in self._cache:
            ...
            for index, multi_modal_data in zip(
                ...
                if index not in multi_modal_inputs_cache:
                    ...
                    # 视频 tensor 已在 dataset.py 中处理好，直接使用
                    ...
                    if len(images) != 0:
                        ...
                        # transformers 新版本里 Qwen2VLImageProcessorFast 不再处理 videos
                        # 这里必须走 video_processor（如果存在），并且把 dataset 侧返回的 (video, nframes) tuple 解包成纯 video tensor
                        ...
                        if hasattr(self.processor, "video_processor") and self.processor.video_processor is not None:
                            multi_modal_inputs = dict(self.processor.video_processor(videos=videos_for_processor, return_tensors="pt"))
                        else:
                            # 兼容老版本 processors：仍允许 image_processor(videos=...)
                            multi_modal_inputs = dict(
                                self.processor.image_processor(images=None, videos=videos_for_processor, return_tensors="pt")
                            )
                    else:
                        multi_modal_inputs = {}
